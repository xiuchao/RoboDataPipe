...
from dataset_io import DATASET_REGISTRY, load_registry
...
ObjectStatusJudge = Callable[[list[dict[str, Any]]], dict[str, Any]]
...
def get_dataset_camera_candidates(dataset: str, registry_path: str) -> list[str]:
    registry = load_registry(registry_path)
    dataset_cfg = registry.get(dataset, {})
    cameras_cfg = dataset_cfg.get("cameras", {}) if isinstance(dataset_cfg, dict) else {}
    embodiment = str(dataset_cfg.get("embodiment", "")) if isinstance(dataset_cfg, dict) else ""
...
def build_online_monitor_config(args: Any) -> OnlineMonitorConfig:
    kwargs: dict[str, Any] = {
        "dataset_name": args.dataset,
        "registry_path": args.registry,
        "prompt_mode": args.prompt_mode,
        "question": args.question,
        "cameras": tuple(args.cameras or get_dataset_camera_candidates(args.dataset, args.registry)),
...
def _detect_gripper_open(signal: np.ndarray, direction: str) -> Event:
    differences = np.diff(np.asarray(signal, dtype=float))
    if len(differences) == 0:
        raise ValueError("Need at least two frames to detect a transition.")
    if direction == "decrease":
        local_index = int(np.argmin(differences) + 1)
        score = float(-np.min(differences))
    else:
        local_index = int(np.argmax(differences) + 1)
...
def _find_gripper_fully_open(
    ...
    *,
    ...
    for local_index in range(max(int(start_idx), 0), len(signal)):
        distance = abs(float(signal[local_index]) - open_value)
        if distance <= open_tolerance:
            return Event(
                name="gripper_fully_open",
                local_index=local_index,
                score=distance,
                method="first_open_value_match",
            )
    return None


def _find_retreat_start(
    position: np.ndarray,
    *,
    start_idx: int,
    window: int,
    ...
    for local_index in range(max(int(start_idx), 0), stop):
        displacement = float(np.linalg.norm(position[local_index + window] - position[local_index]))
        if displacement > displacement_threshold:
            return Event(
                name="retreat_start",
                local_index=local_index,
                score=displacement,
                method="displacement_over_threshold",
            )
    return None
...
class OnlineFailureMonitor:
    def __init__(
        self,
        config: OnlineMonitorConfig,
        object_status_judge: ObjectStatusJudge | None = None,
    ):
        self.config = config
        self.object_status_judge = object_status_judge
        ...
        self.embodiment = str(self.dataset_cfg.get("embodiment", "unknown"))
        ...
        if self.embodiment in {"aibot2", "agibot2"}:
    ...
    def _maybe_capture_left_arm_still(self, items: list[dict[str, Any]]) -> dict[str, Any] | None:
        if self.left_arm_still_event is not None or not items:
            return None
        if self.embodiment not in {"aibot2", "agibot2"}:
            return None
    ...
    def _maybe_capture_gripper_open(self, items: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        open_event = _rename_event(
            _detect_gripper_open(command_signal, direction=self.start_open_signal_spec.open_direction),
            "gripper_start_opening",
        )
        if open_event is None or open_event.local_index <= 0 or open_event.score <= 0.0:
            return None
    ...
    def _maybe_capture_gripper_fully_open(self, items: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        actual_event = _find_gripper_fully_open(
            fully_open_signal,
    ...
    def _maybe_capture_object_status(self, items: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        if self.object_status_judge is None:
            status_event = {
                ...
                "result": {"error": "no object status judge configured"},
                "online_inference_seconds": 0.0,
                "message": "object QA unavailable: no object status judge configured",
            }
        ...
        try:
            result = self.object_status_judge(image_entries)
